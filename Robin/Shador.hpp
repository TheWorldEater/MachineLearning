	...
	for (int y = 0; y<infoheader.biHeight; y++) {
		for (int x = 0; x<infoheader.biWidth; x++) {
			...
			if (1) {
				...
				auto rgb_to_luminosity_coeef = rgbf(0.2989f, 0.5870f, 0.1140f);

				auto tmp = rgb_to_luminosity_coeef * (rgbf)rgb8(bgr[2], bgr[1], bgr[0]) / 255;
				shading.push_back((u8)round((tmp.x +tmp.y +tmp.z) * 255));
			}
		}
	}
	free(src);
...
void readbmp() {
	FILE* pic = fopen("dabs/dab.bmp", "rb");
	if (pic == 0) {
		error = 1;
		return;
	}
