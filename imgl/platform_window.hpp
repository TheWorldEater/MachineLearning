...
#ifdef IMGL_NO_OPENGL // library user can use opengl
...
struct Window {
	...
	draw_func draw_callback = nullptr;
	bool glfw_refresh_callback_called_inside_frame_call = false;
	e_display_mode display_mode;

	static void glfw_refresh_callback (GLFWwindow* window) { // refresh callback so window resizing does not freeze rendering
		Window* obj = (Window*)glfwGetWindowUserPointer(window);
		
		if (obj->glfw_refresh_callback_called_inside_frame_call)
			return; // prevent recursive calls of frame, this once happened when a failed assert tried to open a messagebox

		obj->get_sync_input();

		auto sz = obj->get_size();
		...
		obj->glfw_refresh_callback_called_inside_frame_call = true;

		obj->draw_callback();

		obj->glfw_refresh_callback_called_inside_frame_call = false;

		obj->end_window(obj->display_mode);
	}
