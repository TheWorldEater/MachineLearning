...
union V3 {
	...
	#if FLTVEC
	INL operator u8v3() const;
	INL operator s64v3() const;
	INL operator s32v3() const;
	#endif
	#if INTVEC
	INL operator fv3() const {					return fv3((f32)x, (f32)y, (f32)z); }
	#endif
#endif
};
	...
	INL V3 abs(V3 v) { return V3(imgl::abs(v.x), imgl::abs(v.y), imgl::abs(v.z)); }
	INL T max_component(V3 v) { return imgl::max(imgl::max(v.x, v.y), v.z); }
	...
	INL constexpr V3 map (V3 x, V3 in_a, V3 in_b, V3 out_a, V3 out_b) {
	...
	INL V3 normalize_or_zero (V3 v) { // TODO: epsilon?
		T len = length(v);
		if (len != 0) {
			 v /= len;
		}
	...
	INL V3 mymod (V3 val, V3 range) {
		return V3(	imgl::mymod(val.x, range.x),
					imgl::mymod(val.y, range.y),
					imgl::mymod(val.z, range.z) );
	}
	
	INL V3 floor (V3 v) {						return V3(imgl::floor(v.x),	imgl::floor(v.y),	imgl::floor(v.z)); }
	INL V3 ceil (V3 v) {							return V3(imgl::ceil(v.x),	imgl::ceil(v.y),	imgl::ceil(v.z)); }

	INL V3 pow (V3 v, V3 e) {					return V3(imgl::pow(v.x,e.x),	imgl::pow(v.y,e.y),	imgl::pow(v.z,e.z)); }
	#endif
#endif
