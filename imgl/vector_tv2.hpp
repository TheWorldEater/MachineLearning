...
union V2 {
	...
	#if FLTVEC
	INL operator u8v2() const;
	INL operator s64v2() const;
	INL operator s32v2() const;
	#endif
	#if INTVEC
	INL operator fv2() const {					return fv2((f32)x, (f32)y); }
	#endif
#endif
};
	...
	INL V2 abs (V2 v) { return V2(imgl::abs(v.x), imgl::abs(v.y)); }
	INL T max_component (V2 v) { return imgl::max(v.x, v.y); }
	...
	INL constexpr V2 map (V2 x, V2 in_a, V2 in_b, V2 out_a, V2 out_b) {
	...
	INL V2 normalize_or_zero (V2 v) { // TODO: epsilon?
		T len = length(v);
		if (len != 0) {
			 v /= len;
		}
	...
	INL V2 mymod (V2 val, V2 range) {
		return V2(	imgl::mymod(val.x, range.x),	imgl::mymod(val.y, range.y) );
	}

	INL V2 floor (V2 v) {						return V2(imgl::floor(v.x),	imgl::floor(v.y)); }
	INL V2 ceil (V2 v) {							return V2(imgl::ceil(v.x),	imgl::ceil(v.y)); }

	INL V2 pow (V2 v, V2 e) {					return V2(imgl::pow(v.x,e.x),	imgl::pow(v.y,e.y)); }
	#endif
#endif
